...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $sub_array['status']             = $row['status'];
	  $sub_array['action']		        = ' <button type="button" class="btn btn-warning btn-xs" data-toggle="collapse" data-target=".tampil" onclick="return ubah(\''.$row['id_kebutuhan_produksi'].'\',\''.$row['id_produk'].'\',\''.date('Y-m-d', strtotime($row['tanggal'])).'\',\''.$row['quantity_produksi'].'\',\''.$row['tanggal'].'\',\''.$row['status'].'\')"><i class="ace-icon fa fa-pencil-square-o bigger-120"></i> </button>
