...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $sub_array['nama_file_gambar'] = $row['gambar_produk'];
	  $sub_array['action']        = ' <button type="button" class="btn btn-warning btn-xs" data-toggle="collapse" data-target=".tampil" title="Ubah" onclick="return ubah(\''.$row['id_produk'].'\',\''.$row['nama_produk'].'\',\''.$row['style'].'\',\''.$row['deskripsi'].'\',\''.$row['warna'].'\',\''.$row['harga'].'\')"><i class="ace-icon fa fa-pencil-square-o bigger-120"></i> </button>
