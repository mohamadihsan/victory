...
if(mysqli_escape_string($conn, trim($_POST['hapus']))=='0'){
    $harga_jual          = mysqli_escape_string($conn, trim($_POST['harga_jual']));
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql = "UPDATE detail_supplier
                SET harga_jual='$harga_jual', stok='$stok'
                WHERE id_bahan_baku='$id_bahan_baku' AND id_supplier = '$id_supplier'";
        ...
        // simpan data
        $sql = "INSERT INTO detail_supplier (id_supplier, id_bahan_baku, harga_jual, stok)
                VALUES ('$id_supplier', '$id_bahan_baku', '$harga_jual', '$stok')";
        if(mysqli_query($conn, $sql)){
